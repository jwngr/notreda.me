...
const getGameStats = (gameId) => {
  return getHtmlForUrl(`https://www.sports-reference.com/cfb/boxscores/${gameId}.html`)
    .then(($) => {
      // Stats
      ...
      $statsTable.find('table tbody').find('tr').each((i, row) => {
...
const promises = _.map(yearData, (gameData) => {
  if (!('sportsReferenceGameId' in gameData)) {
    ...
    return getGameStats(gameData.sportsReferenceGameId)
      .catch(error => {
