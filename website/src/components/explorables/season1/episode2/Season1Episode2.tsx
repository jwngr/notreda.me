...
interface InteractiveChartData {
  ...
  readonly seriesData: FirstLossSeriesData[];
}
...
const SliderRangeWrapper = styled(SliderRange)`
  position: absolute;
  top: 50px;
  right: 24px;
`;

const LosslessRecordLineGraphSeasons = styled.p`
  position: absolute;
  top: 20px;
  right: 20px;
  font-size: 20px;
...
export const ExplorablesS1E2: React.FC = () => {
  const [ndInteractiveData, setNdInteractiveData] = useState<InteractiveChartData>({
    startSeason: 1887,
    endSeason: 2017,
    seriesData: [getNdFirstLossSeriesData(1887, 2017)],
  });

  const handleNdSeasonsChange = ([startSeason, endSeason]: readonly number[]): void => {
    setNdInteractiveData({
      startSeason,
      endSeason,
      seriesData: [getNdFirstLossSeriesData(startSeason, endSeason)],
    });
  };

  const [allInteractiveData, setAllInteractiveData] = useState<InteractiveChartData>({
    startSeason: 1917,
    endSeason: 2017,
    seriesData: [getAllTeamFirstLossSeriesData(1917, 2017)],
  });

  const handleAllTeamsSeasonsChange = ([startSeason, endSeason]: readonly number[]): void => {
    setAllInteractiveData({
      startSeason,
      endSeason,
      seriesData: [getAllTeamFirstLossSeriesData(startSeason, endSeason)],
    });
  };
  ...
  if (!staticData || !ndInteractiveData) return null;
  ...
  return (
    <Wrapper>
      ...
      <Paragraph>
        Given those somewhat staggering numbers, one would expect Alabama to have accrued at least a
        few undefeated seasons during Saban's tenure. But they haven't. In fact, they have only gone
        undefeated once in that span, all the way back in 2009.
      </Paragraph>

      <Table
        headers={['Season', 'Record', 'Final AP Ranking']}
        rows={ALABAMA_RECORDS_UNDER_SABAN}
        highlightedRowIndexes={[2]}
      />

      <Caption>
        Despite Alabama's dominance under Nick Saban, the Crimson Tide have only gone undefeated one
      ...
      <Paragraph>
        ...
        nail-biter and blowout wins to climb to the top of the rankings, until they are exposed late
        in the season. Irish fans need only think back to the{' '}
        ...
        while the modern playoff system makes it so that a single loss no longer spells the end of a
        team's National Championship aspirations, there is something memorable about ending the
        season with no losses.
      </Paragraph>

      <Paragraph>
        After a <StyledExternalLink href="/2018/9">road win over Northwestern</StyledExternalLink>,
        Notre Dame sits at #3 in the latest College Football Playoff rankings with an unblemished
        9-0 record, one of only four undefeated teams left in the country (along with #1 Alabama, #2
        Clemson, and #12 UCF). It is nearly impossible to read or listen to anything about Notre
        Dame football without the topic turning to them potentially winning out. While everyone else
        looks ahead, we should not forget that even making it to this point in the season without
        any losses is a feat worth celebrating. Let's dive into the record books to see just how
        impressive it is.
      </Paragraph>
      ...
      <Paragraph>
        ...
        column, the remaining 106 containing at least one loss. Naturally, those losses came spread
        across different points in the season. They lost their opener in some seasons while, in
        others, they did not lose until their postseason bowl game. To get a feel for this, we can
        chart the percentage of all-time seasons in which Notre Dame played a certain number of
        games before suffering its first loss.
      </Paragraph>

      <LosslessRecordLineGraph seriesData={staticData.ndFirstLossSeriesData_1887_2017}>
      ...
      <Paragraph>
        ...
        , they take care of business and make it through the first game unscathed in nearly 9 out of
        every 10 seasons. The numbers drop precipitously from there, with Notre Dame making it 2
        games into the season without a loss in around two-thirds of all seasons and 3 games in just
        a hair under half. From there, it is a somewhat gradual downward slope through 8 games (a
        quarter of all seasons) to 10 games (a seventh of all seasons) to 12 games (only twice, in{' '}
        <StyledExternalLink href="/1988/">1988</StyledExternalLink> and{' '}
      ...
      <Note>
        ...
        Dame played at least that many games. So, although Notre Dame only reached 12-0 twice in 129
        seasons (just 1.5% of all seasons), the chart above only considers the 36 seasons in which
        they have played at least 12 games, resulting in a value of 5.6%.
      </Note>
      ...
      <Paragraph>
        ...
        undefeated at this point in the season. For fans like myself who have only been watching for
        the past couple decades, this may seem pretty surprising. And that would be fair, since
        Irish football is not exactly the behemoth it once was, as evidenced by only looking at
        seasons dating back to 1990.
      </Paragraph>

      <LosslessRecordLineGraph seriesData={staticData.ndFirstLossSeriesData_1990_2017}>
        <LosslessRecordLineGraphSeasons>
          <b>ND:</b> 1990 - 2017
        </LosslessRecordLineGraphSeasons>
      </LosslessRecordLineGraph>
      ...
      <Paragraph>
        Ouch! That looks quick a bit worse. But it is a more accurate picture of the Irish in recent
        times. And it shows that this season's 9-0 record is quite special, happening just two times
        (<StyledExternalLink href="/1993/">1993</StyledExternalLink> and{' '}
        ...
        seasons since Brian Kelly took over in 2010 (remembering that the current 2018 season is not
        included in this data).
      </Paragraph>

      <LosslessRecordLineGraph seriesData={staticData.ndFirstLossSeriesData_2010_2017}>
        <LosslessRecordLineGraphSeasons>
          <b>ND:</b> 2010 - 2017
        </LosslessRecordLineGraphSeasons>
      </LosslessRecordLineGraph>
      ...
      <Paragraph>
        How does this compare to the the peak of the program? Let's look at the same chart covering
        the span of Notre Dame's most dominant dynasty from 1943-1949, predominantly led by head
        coach Frank Leahy. During that seven season stretch, Notre Dame amassed a 60-5-1 record,{' '}
        <StyledExternalLink href="/1943/">won</StyledExternalLink>{' '}
        <StyledExternalLink href="/1946/">four</StyledExternalLink>{' '}
        <StyledExternalLink href="/1947/">National</StyledExternalLink>{' '}
        <StyledExternalLink href="/1949/">Championships</StyledExternalLink>, and opened each season
        with at least five straight victories.
      </Paragraph>

      <LosslessRecordLineGraph seriesData={staticData.ndFirstLossSeriesData_1943_1949}>
        <LosslessRecordLineGraphSeasons>
          <b>ND:</b> 1943 - 1949
        </LosslessRecordLineGraphSeasons>
      </LosslessRecordLineGraph>
      ...
      <LosslessRecordLineGraph seriesData={ndInteractiveData.seriesData}>
        <SliderRangeWrapper
          min={1887}
          max={2017}
          onChange={handleNdSeasonsChange}
          width={140}
          widthSm={140}
        />
        <LosslessRecordLineGraphSeasons>
          <b>ND:</b> {ndInteractiveData.startSeason} - {ndInteractiveData.endSeason}
        </LosslessRecordLineGraphSeasons>
      </LosslessRecordLineGraph>

      <Caption>
        This chart is interactive! Use the slider in the top right to see for yourself how the data
        changes over time.
      </Caption>
      ...
      <PerSeasonBarChart
        data={staticData.ndFirstLossOverTimeBarChartData}
        xAxisLabel="Season"
      ...
      <Paragraph>
        This view of the data shows that Notre Dame is far from its days as a consistent powerhouse.
        Most of the seasons in which it lost one of its opening two games have come in recent
        decades. Notre Dame put together an impressive 36-year run from 1897 to 1932 in which they
        never lost an opener. Compare that to the 1984-86 seasons which all started at least 0-1 and
        a stretch from 1997 to 2011 in which they strung together more than two wins to kick off the
        season just once &mdash; an 8-0 start in{' '}
        <StyledExternalLink href="/2002/">2002</StyledExternalLink>. As was the case with 2012, the
        start of this current 2018 season bucks these recent trends and gives us a glimpse of the
        past.
      </Paragraph>
      ...
      <LosslessRecordLineGraph seriesData={allInteractiveData.seriesData}>
        <SliderRangeWrapper min={1917} max={2017} onChange={handleAllTeamsSeasonsChange} />
        <LosslessRecordLineGraphSeasons>
          <b>All Teams:</b> {allInteractiveData.startSeason} - {allInteractiveData.endSeason}
        </LosslessRecordLineGraphSeasons>
      </LosslessRecordLineGraph>
      ...
      <Paragraph>
        Notre Dame's historical numbers &mdash; and most of the marks under Brian Kelly &mdash; are
        better across the board, except of course for the few teams who have reached the heights of
        13-0 and 14-0. No team has ever achieved a 15-0 record, although it is possible for those
        teams who sometimes schedule an extra 13th regular season game or play in a conference
        championship game.
      </Paragraph>

      <Paragraph>
        Coming back to Alabama's modern-day dynasty, charting their numbers under Saban looks like a
        mountain compared to everyone else's molehills. They have reached 11-0 in over a third of
        those seasons and 12-0 in almost one out of every five. And, as a quirk of typically only
        playing 13 games and having done quite well in postseason contests, they actually see a bump
        from 13 to 14 games.
      </Paragraph>

      <LosslessRecordLineGraph seriesData={staticData.alabamaFirstLossSeriesData_2007_2017}>
        <LosslessRecordLineGraphSeasons>
      ...
      <Caption>
        Alabama under Nick Saban has been extremely consistent, often making it deep into the season
        before losing its first game.
      </Caption>
      ...
      <LosslessRecordLineGraph
        seriesData={[
          ...staticData.ndFirstLossSeriesData_1887_2017,
          ...staticData.ndFirstLossSeriesData_1990_2017,
          ...staticData.ndFirstLossSeriesData_2010_2017,
          ...staticData.ndFirstLossSeriesData_1943_1949,
          ...staticData.alabamaFirstLossSeriesData_2007_2017,
          ...staticData.allFirstLossSeriesData_1917_2017,
        ]}
        showArea={false}
      ...
      <Paragraph>
        As dominant as modern-day Alabama is, the Notre Dame dynasty from 1943-49 was even more so,
        at least on this metric. Meanwhile, Notre Dame teams since 1990 trend just a few percentage
        points above the all team average. Under Brian Kelly, the Irish's numbers look generally
        healthier, although they were certainly buoyed by the great 2012 season. After factoring in
        whenever the Irish first fall this season &mdash; if they do at all &mdash; his numbers will
        look even better.
      </Paragraph>
      ...
      <Paragraph>
        ...
        &mdash; the first season of intercollegiate football in the United States &mdash; 431 teams
        have gone undefeated, spread across 109 unique schools. Nearly a third of those 149 football
        seasons ended with four or more undefeated teams, including two seasons &mdash; 1910 and
        1920 &mdash; which saw a record ten undefeated teams.
      </Paragraph>

      <Table
        headers={['Undefeated Teams', 'Seasons', 'Latest']}
        rows={staticData.undefeatedTeamCountsPerSeasonTableData}
      />
      ...
      <Note>
        An undefeated season is considered any season, regardless of length, in which a team did not
        have a single loss, even if that team had one or more ties.
      </Note>

      <Paragraph>
        It has been 46 years since a season ended with four or more undefeated teams. That
        particular 1973 season was noteworthy for Notre Dame as they claimed a National Championship
        with a{' '}
      ...
      <PerSeasonBarChart
        data={staticData.undefeatedTeamCountsPerSeasonBarChartData}
        xAxisLabel="Season"
      ...
      <Table
        ...
        rows={staticData.undefeatedSeasonCountsPerTeamTableData_1869_2017}
        highlightedRowIndexes={[2]}
      />
      ...
      <Paragraph>
        The table above may be accurate according to the official records, but it does not take into
        account that Princeton, Yale, Harvard, and even Notre Dame are some of the oldest football
        programs and went lossless in seasons in which they only played a handful of games. In fact,
        in Princeton's first six undefeated seasons, it played a total of just 10 games, fewer than
      ...
      <Table
        ...
        rows={staticData.undefeatedSeasonCountsPerTeamTableData_1917_2017}
      ...
      <Paragraph>
        Notre Dame sits at 9-0, five wins away from the school's 24th undefeated season. It would be
        its first since they last{' '}
        ...
        Irish in rare company. In fact, the Irish have started this season with as many wins as they
        finished with in all but a handful of their all-time undefeated seasons.
      </Paragraph>

      <Table
        headers={['Season', 'Games Played', 'Record']}
        rows={staticData.ndUndefeatedSeasonTableData}
        highlightedRowIndexes={[11, 13, 18, 21, 22]}
      />

      <Caption>
        Notre Dame's win count at this point in the 2018 season is already as high as it was in all
        but five of its 23 all-time undefeated seasons.
      </Caption>
